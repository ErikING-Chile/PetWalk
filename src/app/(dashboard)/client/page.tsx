...
import { Dog, MapPin, Clock, Calendar, ChevronRight, ShoppingBag, Plus, Wallet } from "lucide-react"
    ...
    return (
        <div className="space-y-6 pb-20">
            ...
            <div className="grid grid-cols-2 gap-4 auto-rows-fr">
                <Link href="/client/book" className="block h-full">
                    <button className="w-full h-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-4 px-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-200 flex flex-col items-center justify-center gap-2 group">
                        <div className="flex items-center gap-2">
                            <Plus className="w-5 h-5 group-hover:rotate-90 transition-transform duration-300" />
                            <span className="text-base">Agendar Paseo</span>
                        </div>
                        <span className="text-xs text-purple-100 opacity-80">Encuentra walkers cerca</span>
                    </button>
                </Link>
                <Link href="/client/marketplace" className="block h-full">
                    <button className="w-full h-full bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white font-bold py-4 px-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-200 flex flex-col items-center justify-center gap-2">
                        <div className="flex items-center gap-2">
                            <ShoppingBag className="w-5 h-5" />
                            <span className="text-base">Marketplace</span>
                        </div>
                        <span className="text-xs text-blue-100 opacity-80">Productos para mascotas</span>
                    </button>
                </Link>
            </div>
            ...
            <div className="flex gap-3">
                <Link href="/client/payment-methods" className="flex-1 block h-full">
                    <button className="w-full h-full bg-white/5 hover:bg-white/10 text-white py-3 px-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 border border-white/10">
                        <Wallet className="w-4 h-4" />
                        <span className="text-sm font-medium">Métodos de Pago</span>
                    </button>
                </Link>
                <Link href="/client/pets" className="flex-1 block h-full">
                    <button className="w-full h-full bg-white/5 hover:bg-white/10 text-white py-3 px-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 border border-white/10">
                        ...
                        <div className="flex justify-between items-start">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center text-xs text-purple-300 gap-1">
                                        {/* @ts-ignore */}
                                        <span>Con {nextWalk.walker?.profiles?.full_name || 'Un Walker'}</span>
                                    </div>
                                </div>
                            </div>
                            <span className={`px-3 py-1 rounded-full text-xs font-bold capitalize border ${nextWalk.status === 'assigned' ? 'bg-blue-500/20 text-blue-400 border-blue-500/20' :
                                ...
                                {nextWalk.status === 'requested' ? 'Buscando...' :
                                    nextWalk.status === 'assigned' ? 'Walker Asignado' :
                        ...
                        {nextWalk.start_code && nextWalk.status !== 'in_progress' && (
                            <div className="mt-4 p-4 bg-blue-500/10 rounded-xl border border-blue-500/20 flex flex-col items-center justify-center text-center animate-pulse">
                                <p className="text-xs text-blue-300 font-bold uppercase tracking-widest mb-2">
                                    {nextWalk.status === 'assigned' ? '¡Tu walker viene en camino!' : 'Código de Inicio'}
                                </p>
                        ...
                        {(nextWalk.status === 'in_progress' || nextWalk.status === 'assigned') && (
                            <Link href={`/client/track/${nextWalk.id}`} className="mt-4 block">
                                <div className={`w-full border rounded-lg py-3 flex items-center justify-center gap-2 font-bold transition-all shadow-lg animate-pulse ${nextWalk.status === 'in_progress'
                                    ...
                                    <MapPin size={18} />
                                    {nextWalk.status === 'in_progress' ? "VER MAPA EN VIVO" : "VER WALKER Y CHAT"}
                                </div>
                            </Link>
                        )}
